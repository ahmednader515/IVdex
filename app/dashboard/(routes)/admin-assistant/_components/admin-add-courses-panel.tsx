...
export function AdminAddCoursesPanel({ embedded = false }: { embedded?: boolean }) {
    ...
    useEffect(() => {
        ...
        const fetchOwned = async () => {
            ...
            try {
                const res = await fetch(`/api/admin-assistant/users/${selectedUser.id}/courses`);
                if (res.ok) {
    ...
    const handleAddCourse = async () => {
        ...
        try {
            const response = await fetch(`/api/admin-assistant/users/${selectedUser.id}/add-course`, {
                method: "POST",
                headers: {
    ...
    const handleDeleteCourse = async () => {
        ...
        try {
            const res = await fetch(`/api/admin-assistant/users/${selectedUser.id}/add-course`, {
                method: "DELETE",
                ...
                <CardContent>
                    ...
                    <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                        <div className="text-sm text-muted-foreground">
                            Showing {filteredUsers.length === 0 ? 0 : start + 1}-{Math.min(start + pageSize, filteredUsers.length)} of {filteredUsers.length}
                        </div>
